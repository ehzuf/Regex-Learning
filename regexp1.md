...
*字符集*。字符集，例如 [0-9] 或 \w 或 . (dot)，就是并接的简洁表现形式。字符集可以在编译过程中扩展成并接形式，尽管用一种新的NFA类型明确地表示它们会更加有效。[POSIX](http://www.opengroup.org/onlinepubs/009695399/basedefs/xbd_chap09.html)定义了特殊的字符集诸如[[:upper:]] ，可以根据当前的位置改变含义。但是这部分困难的地方是确定它们的意思，而不是将意思编码如NFA。
